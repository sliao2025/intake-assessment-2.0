...
export default function ReportSectionAdult({
  ...
  const adultAssess = profile.assessments.data as AdultAssessments;
  ...
  const aceScore = scoreSum(adultAssess?.aceResilience as Record<string, any>);
  ...
  const crafftB = adultAssess?.crafft?.partB ?? ({} as Record<string, any>);
  ...
  const [interp, setInterp] = useState<ReportInterpretations>(() => {
    ...
    if (stored && "gad7" in stored) {
      return stored as ReportInterpretations;
    }
    return coerceInterp(undefined);
  });
